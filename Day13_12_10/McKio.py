...
# 시간대에 맞는 카테고리 반환
def get_time_period(hour2, minute2):
    if (4, 0) <= (hour2, minute2) < (10, 30):
        return "morning"
    elif (10, 30) <= (hour2, minute2) < (14, 0):
        return "lunch"
    else:
        return "dinner"
...
# 필터링 함수
def filter_menu(burger_dict, cate):  # 버거딕셔너리, 카테고리
    filtered_menu = []

    for name, details in burger_dict.items():  # 버거딕셔너리에서 이름, 정보 받아옴
        if cate in details[1]:  # 이건 이제 그거 뭐냐 b하고 c 거름 (비프/치킨)
...
def set_upgrade(selection, my_cart):
    ...
    if set_how == '1':
        ...
        if size == '2':  # 라지 선택 while문으로 감싸서 에러처리하면 더 좋긴할듯
            set_price += large_price

        print(f"세트 추가 비용: +{set_price}원")

        # 세트메뉴 구성 가격계산 뭐 어떻게 해야할지 몰라서 걍 더해버림
        ...
        # 세트 딕셔너리 (이름도 불고기 -> 불고기 세트 이렇게 바껴야하니까)
        set_item = {
            "name": updated_name,
            "drink": chosen_drink,
            "side": chosen_side,
...
# 가격 계산 함수 뭔가 방법이 없을까 뭔가 뭔가 비효율적으로 보이는데
def calculate(my_cart):
    ...
    for item in my_cart:
        ...
        elif item in mcMorning:
            total_price += mcMorning[item][0]
        elif item in mcLunch:
            total_price += mcLunch[item][0]
        else:
...
# __name__() __main__ 아래에서 부터 메인루프
